...
def create_dataloader(config: PretrainConfig, split: str, rank: int, world_size: int, **kwargs):
    dataset = PuzzleDataset(
        PuzzleDatasetConfig(seed=config.seed, dataset_path=config.data_path, rank=rank, num_replicas=world_size, **kwargs),
        split=split,
    )
...
def create_model(config: PretrainConfig, train_metadata: PuzzleDatasetMetadata, world_size: int):
    ...
    model_cls = getattr(hrm, config.arch.name.split("@")[1])  # type: ignore
    loss_head_cls = loss.ACTLossHead
    ...
    with torch.inference_mode():
        ...
        if metric_values is not None:
            ...
            if rank == 0:
                ...
                reduced_metrics = {
                    set_name: {metric_name: reduced_metrics[set_id, metric_id] for metric_id, metric_name in enumerate(metric_keys)} for set_id, set_name in enumerate(set_ids)
                }
...
def save_code_and_config(config: PretrainConfig):
    ...
    # Copy code
    module = hrm  # type: ignore
    code_list = [
        inspect.getsourcefile(hrm),
        inspect.getsourcefile(loss),
    ]
