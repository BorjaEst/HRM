...
class CastedSparseEmbeddingSignSGDConfig(BaseModel, extra="forbid"):
    ...
    world_size: int = Field(
        default=1,
        description="World size for distributed training. If greater than 1, the optimizer performs all-gather and unique operations across all processes to ensure consistent updates of the sparse embedding weight.",
    )
...
class CastedSparseEmbeddingSignSGD_Distributed(Optimizer):
    ...
    def step(self, closure=None):  # type: ignore
        for group in self.param_groups:
            # Find the sparse embedding weight
    ...
    def _sparse_emb_signsgd_dist(
        ...
        if world_size > 1:
            ...
            all_ids = torch.empty(world_size * N, dtype=local_indices.dtype, device=local_indices.device)

            dist.all_gather_into_tensor(all_weights_grad, local_weights_grad)
            dist.all_gather_into_tensor(all_ids, local_indices)
