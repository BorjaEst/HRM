...
from hrm_sn.models.trm_v1 import Model, ModelConfig_TRM_V1
from hrm_sn.modules.trm import TRMConfig
...
CONFIGURATION_PATH = os.environ.get("EXP02_CONFIGURATION_PATH", "config/defaults_trm.toml")
...
class RunArguments(BaseSettings, extra="forbid", cli_parse_args=True):
    """Root experiment settings for TRM baseline training.
    ...
    run_name: Optional[str] = Field(
        ...
        description=(
            ...
            "(e.g. `TrmV1 2x128 4L 16H 0.1D ACT-torch cool-slug`)."
        ),
    )

    # ---------------------------------------------------------------------------------------------
    # Model architecture and data
    architecture: TRMConfig = Field(
        ...,
        description=(
            "Architecture config for the TRM model. The keys in `architecture` are passed to the"
            "TRModel constructor."
        ),
    )
    # FIXME WE REMOVED ACT CONTROLLER FROM HERE AS TRM DOES NOT USE ACT

    loss: Something = Field(
        ...,
    ...
    @property
    def model(self) -> ModelConfig_TRM_V1:
        """Compose ModelConfig_TRM_V1 from leaf settings."""
        return ModelConfig_TRM_V1.model_validate(self, from_attributes=True)
